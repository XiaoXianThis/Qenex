...
服务启动后监听 `http://localhost:8000`（端口由配置决定），**同一端口**提供 Web UI 与 REST/SSE API。前端 `apps/web/dist` 在编译时通过 `rust-embed` 嵌入二进制；若 dist 不存在，`build.rs` 会写入占位页并提示先运行 `bun run build`。

日志级别可通过 `RUST_LOG` 调整，例如：
...
```toml
[dependencies]
acp-to-agui = { path = "../crates/bridge" }

# 需要内置 HTTP 服务时
acp-to-agui = { path = "../crates/bridge", features = ["server"] }
```
